/* Ukrainischkurs für Joel · Wortbetonung v3
   Verifizierte Betonungen für häufige A1-Wortformen. Die Freischaltung wird
   über den zentralen Lernkern aus den echten Kurskarten ermittelt. */
(()=>{
  ...
  function legacyIntroDay(entry){const needle=canon(entry.plain);for(let di=0;di<D.length;di++){const cards=D[di]?.[3]||[];if(cards.some(card=>(' '+canon(card?.[0])+' ').includes(' '+needle+' ')))return di}return -1}
  function introDay(entry){return core?core.introductionDay(entry.plain,{cardOnly:true}):legacyIntroDay(entry)}
  ...
  function options(entry){const chars=Array.from(strip(entry.plain)),variants=[];chars.forEach((ch,i)=>{if(!VOWELS.includes(ch))return;const c=[...chars];c.splice(i+1,0,'\u0301');variants.push(c.join('').normalize('NFC'))});const correct=entry.marked.normalize('NFC'),wrong=shuffle(variants.filter(x=>x!==correct));return shuffle([correct,...wrong.slice(0,2)])}
  let session=null;
  ...
  function renderBox(){let box=document.getElementById('wordStressBox');if(!required()){if(box)box.hidden=true;return}const cards=document.getElementById('cards');if(!cards)return;if(!box){box=document.createElement('section');box.id='wordStressBox';box.className='card';cards.insertAdjacentElement('afterend',box)}box.hidden=false;const st=state(),fresh=newToday();if(session){const q=session.items[session.idx];box.innerHTML='<div class="label">Betonung · '+(session.idx+1)+' / '+session.items.length+'</div><h2>Welche Schreibweise zeigt die richtige Betonung?</h2><div class="ws-plain" lang="uk">'+strip(q.plain)+'</div><div class="ws-grid">'+options(q).map(x=>'<button class="answer" data-ws="'+x+'" lang="uk">'+x+'</button>').join('')+'</div><p class="small">Der Akzentstrich ist hier nur Lernmarkierung.</p>';box.querySelectorAll('[data-ws]').forEach(b=>b.onclick=()=>answer(b.dataset.ws));return}const pool=fresh.length?fresh:eligible();box.innerHTML='<div class="label">'+(fresh.length?'Neue Wortbetonung':'Review · Wortbetonung')+'</div><h2>Betonung nicht dem Zufall überlassen</h2><p class="small">Trainiert werden ausschließlich bereits eingeführte Wortformen mit geprüftem Akzent. Verschiebt sich der Kurs später, verschiebt sich diese Aufgabe automatisch mit.</p><div class="ws-known">'+(st.passed?'✓ Für heute bestanden.':'Heute: '+(fresh.length?fresh.length+' neue verifizierte Wortformen':'6 gemischte Wortformen; höchstens ein Fehler')+'.')+'</div><div class="small">'+sourceSummary(pool)+' · Akzentzeichen sind Lernhilfe und keine normale Schreibpflicht.</div><div class="actions"><button class="'+(st.passed?'secondary':'primary')+'" id="wsStart">'+(st.passed?'noch einmal':'Betonung starten')+'</button></div>';document.getElementById('wsStart').onclick=start}
