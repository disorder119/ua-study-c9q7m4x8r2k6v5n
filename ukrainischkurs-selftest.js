/* Ukrainischkurs für Joel · Laufzeit-Selbsttest v19 */
(() => {
  ...
  try{
    ...
    ok(window.UKRAINIAN_COURSE_LOADER?.version===30,'Loader ist nicht auf v30');ok(window.UKRAINIAN_COURSE_LOADER?.evalFree===true,'Loader meldet keinen eval-freien Modus');ok(window.UKRAINIAN_COURSE_LOADER?.mode==='classic-script','Loader nutzt nicht den klassischen Skriptmodus');
    ...
    ok(!!s.conversationChain,'Gesprächsketten fehlen');ok(Number(s.conversationChain.version)>=1,'Gesprächsketten sind nicht auf v1');
    ...
    ok(typeof scheduleMeta==='function'&&typeof dueCards==='function','Adaptives SRS fehlt');ok(D.length>=76,'Grundkurs enthält weniger als 76 geführte Tage');ok(WEEKLY_REVIEW_DAYS.includes(D.length-1),'Letzter Kurstag ist nicht als Review markiert');ok(document.getElementById('next'),'Weiter-Button fehlt');
