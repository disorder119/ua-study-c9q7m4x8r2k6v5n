/* Ukrainischkurs für Joel · Laufzeit-Selbsttest v18 */
(() => {
  ...
  try{
    ...
    ok(window.UKRAINIAN_COURSE_LOADER?.version===29,'Loader ist nicht auf v29');ok(window.UKRAINIAN_COURSE_LOADER?.evalFree===true,'Loader meldet keinen eval-freien Modus');ok(window.UKRAINIAN_COURSE_LOADER?.mode==='classic-script','Loader nutzt nicht den klassischen Skriptmodus');
    ...
    ok(!!s.a1CanDo,'A1-Can-do-Abschluss fehlt');ok(Number(s.a1CanDo.version)>=4,'Can-do ist nicht auf v4');
    ...
    ok(typeof scheduleMeta==='function'&&typeof dueCards==='function','Adaptives SRS fehlt');ok(D.length>=70,'Grundkurs enthält weniger als 70 geführte Tage');ok(WEEKLY_REVIEW_DAYS.includes(D.length-1),'Letzter Kurstag ist nicht als Review markiert');ok(document.getElementById('next'),'Weiter-Button fehlt');
