/* Ukrainischkurs für Joel · Laufzeit-Selbsttest v9 */
(() => {
  ...
  try{
    ...
    ok(!!s.grammarSpiral,'Grammatik-Spirale fehlt');ok(Number(s.grammarSpiral.version)>=1,'Grammatik-Spirale ist nicht aktiv');
    ok(!!s.storyLab,'Mini-Geschichten fehlen');ok(Number(s.storyLab.version)>=1,'Story Lab ist nicht aktiv');
    ok(!!s.dictation,'Hör-Diktat fehlt');ok(Number(s.dictation.version)>=1,'Hör-Diktat ist nicht aktiv');
