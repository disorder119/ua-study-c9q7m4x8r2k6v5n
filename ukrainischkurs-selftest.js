/* Ukrainischkurs für Joel · Laufzeit-Selbsttest v7 */
(() => {
  ...
  try{
    ...
    ok(!!s.readingTransfer,'Lese-Transfer fehlt');ok(!!s.foundationExpansion,'Grundkurs-Erweiterung fehlt');ok(!!s.a1Expansion2,'A1-Erweiterung 2 fehlt');
    ok(!!s.comprehensionLab,'Verständnis-Labor fehlt');ok(!!s.activeProduction,'Aktive Produktion fehlt');ok(!!s.a1CanDo,'A1-Can-do-Abschluss fehlt');
    ok(typeof scheduleMeta==='function'&&typeof dueCards==='function','Adaptives SRS fehlt');
    ok(D.length>=57,'Grundkurs enthält weniger als 57 geführte Tage');
    ok(WEEKLY_REVIEW_DAYS.includes(D.length-1),'Letzter Kurstag ist nicht als Review markiert');
