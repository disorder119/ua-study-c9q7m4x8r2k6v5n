...
(()=>{
  ...
  function answer(v){const q=session.items[session.idx],good=q.a.map(norm).includes(norm(v));if(good)session.correct++;else if(q.critical)session.criticalMiss=true;toast(good?'Richtig.':'Muster: '+q.a[0]);session.idx++;if(session.idx>=session.items.length){const st=state(),score=Math.round(session.correct/session.items.length*100),passed=session.correct>=Math.max(1,session.items.length-1)&&!session.criticalMiss;st.best=Math.max(st.best||0,score);st.attempts++;st.date=date();st.passed=passed;save();session=null;toast(passed?'Grammatik-Spirale bestanden.':'Noch nicht stabil: maximal ein unkritischer Fehler, kritische Alltagssätze müssen sitzen.');render();return}renderBox()}
