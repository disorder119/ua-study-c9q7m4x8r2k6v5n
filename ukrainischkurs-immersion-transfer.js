...
(()=>{
  ...
  const LESSONS=SCENARIOS.map((x,i)=>[x.title,'Bekannte Sprache zunehmend ohne deutsche Stütze aktiv benutzen.',x.intro,[]]);
  ...
  let session=null;
  function begin(){const sc=scenario();if(!sc)return;session={items:sc.items,idx:0,correct:0,assisted:false,history:[],started:true,revealed:false};renderBox()}
  ...
  function answer(value){
    const item=current();if(!item)return;const good=item.a.map(norm).includes(norm(value));session.history.push({q:item.q,a:value||'—',good});if(good)session.correct++;
    toast(good?'Підходить.':'Правильний варіант: '+item.a[0]);session.idx++;
    if(session.idx>=session.items.length){finish();return}
    session.revealed=false;renderBox()
  }
