...
assert(loader.includes("const VERSION='43'"),'Loader ist nicht v43');
for(const token of ['adaptive-alphabet.js?v=2','designer-alphabet.js?v=1','alphabet-proof.js?v=2','learning-core.js?v=4','a1-exam.js?v=2','a1-cando.js?v=7','selftest.js?v=32'])assert(loader.includes(token),`Loader vermisst ${token}`);
...
assert(designer.includes('const VERSION=1'),'Designer-Alphabet ist nicht v1');
...
for(const token of ['Alexander McQueen','Ann Demeulemeester','Balenciaga','Balmain','Vivienne Westwood','Jean Paul Gaultier','Yohji Yamamoto','Margiela','Ottolinger','Prada','Raf Simons','Rundholz','Saint Laurent','гаманець','куртка','спідниця','штани','шкіра','ціна','черевики'])assert(designer.includes(token),`Persönlicher Mode-/Shop-Merkanker fehlt: ${token}`);
...
assert(designer.includes("quiz.correct===quiz.letters.length")&&designer.includes("Merkcheck"),'Aktiver 3er-Merkcheck fehlt');
assert(designer.includes('Eselsbrücken für das Zeichen')&&designer.includes('Für die Aussprache zählt immer'),'Merkanker werden nicht sauber von echter Aussprache getrennt');
...
assert(selftest.includes('Laufzeit-Selbsttest v32')&&selftest.includes('version===43')&&selftest.includes('UKRAINIAN_DESIGNER_ALPHABET')&&selftest.includes('exam?.doublePass===true'),'Laufzeit-Selbsttest ist nicht auf v43/Designer-Alphabet/Doppelprüfung');

if(errors.length){console.error(`VALIDIERUNG FEHLGESCHLAGEN (${errors.length})`);errors.forEach(e=>console.error('- '+e));process.exit(1)}
console.log('VALIDIERUNG OK: v43 mit persönlichem Disorder119-Designer-Alphabet über alle 33 Zeichen, drei passenden Merkhaken pro Einführungstag, aktivem 3er-Merkcheck, echten Buchstaben-Audioreferenzen und unverändert strenger Alphabet-/A1-Mastery geprüft.');