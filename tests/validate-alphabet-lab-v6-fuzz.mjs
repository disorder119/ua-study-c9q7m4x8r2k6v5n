...
let state=C.freshState(),previousMain=0,previousCoverage=0,now=NOW,session=null;
...
for(let i=0;i<2500;i++){
  ...
  if(!session||session.mainIndex>=session.targetMainCount)session=C.createAdaptiveSession(state,{targetMainCount:20,title:'fuzz',preset:'fuzz',scope:'standard',feedback:'learning'});
  session.fixedLetters=[letter];
  const task=C.selectNextMainQuestion(state,session,rng,now)||C.buildV4Task(state,letter,skill,Math.min(3,C.difficultyFor(state,letter,skill,now)),'visual-to-sound',session,rng,{scheduledReason:'fuzz'});
  ...
  session.mainAnswers.push({letter:task.letter,skill:task.skill,family:task.family,correct:good});session.mainTasks.push(task);session.mainIndex++;
