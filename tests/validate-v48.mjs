...
assert(sw.includes("const VERSION='48'"),'Service Worker ist nicht v48');
for(const asset of ["'./ukrainischkurs-learning-state-guard.js'","'./ukrainischkurs-fashion-bridge.js'","'./ukrainischkurs-resale-practice.js'","'./ukrainischkurs-spoken-transfer.js'","'./ukrainischkurs-a1-exam.js'"])assert(sw.includes(asset),`Offline-Asset fehlt: ${asset}`);
...
// Verhalten der Lernstatus-Härtung in einem kleinen isolierten Zustandsmodell prüfen.
...
const ctx={console,window:{},s:state,D:Array.from({length:20},()=>['','','',[]]),date:()=>today,blank,save(){},study(){if(!state.dates.includes(today))state.dates.push(today)},lessonComplete(di){const p=state.lessonProgress[di];return !!(p&&p.testPassed&&p.spoken&&p.reviewDone)},syncLesson(di){const p=state.lessonProgress[di];if(p&&p.testPassed&&p.spoken&&p.reviewDone)state.done[di]=true;else delete state.done[di]},syncLessons(){},render(){},resetProgress(){},alphabetReady(){return true},requireAlphabet(){return true},completedLessons(){let n=0;while(state.done[n])n++;return n},$(){return null},toast(){},show(){},document:{createElement(){return {className:'',innerHTML:'',title:'',onclick:null,append(){}}}}};
