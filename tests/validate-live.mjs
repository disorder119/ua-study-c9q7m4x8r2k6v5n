...
const flowHardening=fs.readFileSync(path.join(root,'ukrainischkurs-guided-flow-hardening.js'),'utf8');
...
assert(realSw.includes("const VERSION='68'"),'Live-Service-Worker ist nicht v68');
for(const asset of ['./ukrainischkurs-simple-foundation.js','./ukrainischkurs-guided-start.js','./ukrainischkurs-guided-writing-hardening.js','./ukrainischkurs-guided-flow-hardening.js','./ukrainischkurs-audio-quality-gate.js','./ukrainischkurs-a1-human-listening-gate.js'])assert(realSw.includes(`'${asset}'`),`${asset} fehlt im Offline-Cache`);
...
assert(loader.includes("await loadScript('./ukrainischkurs-guided-flow-hardening.js?v=1'"),'Geführter Tageswechsel-Fix wird nicht geladen');
...
assert(loader.indexOf('guided-start.js?v=4')<loader.indexOf('guided-writing-hardening.js?v=1')&&loader.indexOf('guided-writing-hardening.js?v=1')<loader.indexOf('guided-flow-hardening.js?v=1'),'Geführte Anfänger-Härtungen laden nicht in sicherer Reihenfolge');
...
assert(flowHardening.includes("if((Number(s.day)||0)>before)"),'Flow-Fix darf neuen Guided-State erst nach erfolgreichem bestehendem Tageswechsel anfassen');
...
assert(a1HumanListening.includes('Kein TTS-Ersatz in der Prüfung'),'Fehlendes Human-Audio muss hart ohne TTS-Fallback scheitern');
...
try{
  fs.writeFileSync(swPath,realSw.replace("const VERSION='68'","const VERSION='58'"),'utf8');
...
console.log(`LIVE-VALIDIERUNG OK: v68 hat ${coverage.toFixed(1)}% (${exact.length}/${guidedPairs.size}) exakt passende menschliche Anfänger-Audios; Druckschrift-Nachmalen verlangt Formnähe + räumliche Abdeckung; der tote Anfänger-Weiterbutton wird über den bestehenden Tages-Gate-Handler repariert; Aussprache-Coach/Mastery sind TTS-frei; A1 Hören braucht zwei Human-only 9/10-Nachweise.`);