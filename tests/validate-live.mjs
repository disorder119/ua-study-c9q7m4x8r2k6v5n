...
assert(realSw.includes("const VERSION='61'"),'Live-Service-Worker ist nicht v61');
...
assert(loader.includes("await loadScript('./ukrainischkurs-guided-start.js?v=2'"),'Geführter Alphabet-Start v2 wird nicht geladen');
assert(loader.includes("./ukrainischkurs-native-audio.js?v=4"),'Neue verifizierte Audioanker werden nicht geladen');
assert(loader.indexOf('simple-foundation.js?v=1')<loader.indexOf('guided-start.js?v=2'),'Geführter Start muss nach der bisherigen Grundlagen-UI laden');
for(const marker of ['oneScreenOneTask:true','pictureLearning:true','tracing:true','humanAudioOnly:true','strokeByStroke:true','autoHelp:true','easyFirstChoice:true','guided-trace','Los geht’s','Noch einen lernen','Bleib auf der hellen Form'])assert(guided.includes(marker),`Geführter Start vermisst ${marker}`);
...
try{
  fs.writeFileSync(swPath,realSw.replace("const VERSION='61'","const VERSION='58'"),'utf8');
...
console.log('LIVE-VALIDIERUNG OK: v61 schützt den maximal einfachen bildlichen Alphabet-Start mit Human-Audio, Strich-für-Strich-Nachmalen, leichter Erstauswahl und automatischer Hilfe.');