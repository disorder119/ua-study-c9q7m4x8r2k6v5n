...
const a1HumanListening=read('ukrainischkurs-a1-human-listening-gate.js'),a1Writing=read('ukrainischkurs-a1-writing-quality-gate.js'),a1HumanSpeaking=read('ukrainischkurs-a1-human-speaking-gate.js'),a1Evidence=read('ukrainischkurs-a1-evidence-gate.js');

assert(realSw.includes("const VERSION='70'"),'Live-Service-Worker ist nicht v70');
for(const asset of ['./ukrainischkurs-simple-foundation.js','./ukrainischkurs-guided-start.js','./ukrainischkurs-guided-writing-hardening.js','./ukrainischkurs-guided-flow-hardening.js','./ukrainischkurs-audio-quality-gate.js','./ukrainischkurs-a1-human-listening-gate.js','./ukrainischkurs-a1-writing-quality-gate.js','./ukrainischkurs-a1-human-speaking-gate.js','./ukrainischkurs-a1-evidence-gate.js'])assert(realSw.includes(`'${asset}'`),`${asset} fehlt im Offline-Cache`);
for(const marker of ['./ukrainischkurs-native-audio.js?v=6','./ukrainischkurs-pronunciation.js?v=5','./ukrainischkurs-pronunciation-mastery.js?v=5','./ukrainischkurs-audio-quality-gate.js?v=2','./ukrainischkurs-human-listening.js?v=4','./ukrainischkurs-spoken-transfer.js?v=2','./ukrainischkurs-a1-human-listening-gate.js?v=1','./ukrainischkurs-a1-writing-quality-gate.js?v=1','./ukrainischkurs-a1-human-speaking-gate.js?v=1','./ukrainischkurs-a1-evidence-gate.js?v=2','./ukrainischkurs-guided-start.js?v=4','./ukrainischkurs-guided-writing-hardening.js?v=1','./ukrainischkurs-guided-flow-hardening.js?v=1'])assert(loader.includes(marker),`Loader vermisst ${marker}`);
...
assert(loader.indexOf('a1-exam.js?v=2')<loader.indexOf('a1-human-listening-gate.js?v=1')&&loader.indexOf('a1-human-listening-gate.js?v=1')<loader.indexOf('a1-writing-quality-gate.js?v=1')&&loader.indexOf('a1-writing-quality-gate.js?v=1')<loader.indexOf('a1-human-speaking-gate.js?v=1')&&loader.indexOf('a1-human-speaking-gate.js?v=1')<loader.indexOf('a1-evidence-gate.js?v=2')&&loader.indexOf('a1-evidence-gate.js?v=2')<loader.indexOf('a1-cando.js?v=7'),'A1-Zusatzgates laden nicht zentral vor Can-do');
...
for(const marker of ['tasksPerAttempt:4',"threshold:'4/4'",'doublePass:true','differentCalendarDays:true','humanQuestionAudioOnly:true','noTtsFallback:true','recordingRequired:true','replayRequired:true','intelligibilityEvidenceRequired:true','questionTextHidden:true','blocksA1Milestone:true','session.plays>=2','session.recorded','session.replayed','session.evidence',"module:'a1-human-speaking-",'weight:2.75','SpeechRecognition','reviewerCheck','Special:FilePath','Commons-Dateiseite'])assert(a1HumanSpeaking.includes(marker),`A1 Human Speaking vermisst ${marker}`);
for(const phrase of ['Як тебе звати?','Як це називається?','Скільки це коштує?','Котра година?'])assert(a1HumanSpeaking.includes(phrase),`A1 Human Speaking vermisst Human-Frage ${phrase}`);
assert((a1HumanSpeaking.match(/\{id:'/g)||[]).length===4,'A1 Human Speaking braucht exakt vier kontrollierte Human-Fragen');
assert(!a1HumanSpeaking.includes('speechSynthesis')&&!a1HumanSpeaking.includes('SpeechSynthesisUtterance')&&!a1HumanSpeaking.includes('speak('),'A1 Human Speaking darf keinen TTS-/speak-Fallback enthalten');
assert(a1HumanSpeaking.includes('session.correct===total')&&a1HumanSpeaking.includes('st.qualification.date!==st.confirmation.date'),'A1 Human Speaking erzwingt weder 4/4 noch unabhängige Kalendertage');
assert(a1HumanSpeaking.includes('Kein TTS-Ersatz in der Prüfung'),'A1 Human Speaking muss Human-Audio-Ausfall hart behandeln');

for(const marker of ['centralized:true','requiresOriginalDoublePass:true','requiresHumanListeningDoublePass:true','requiresStructuredWritingDoublePass:true','requiresHumanSpeakingDoublePass:true',"core.registerMilestone?.('a1.exam'",'originalDomains()','humanListening()','writingQuality()','humanSpeaking()','r.originalDomains&&r.humanListening&&r.writingQuality&&r.humanSpeaking'])assert(a1Evidence.includes(marker),`Zentrales A1-Evidence-Gate vermisst ${marker}`);
assert(a1Evidence.includes('const VERSION=2'),'A1 Evidence Gate ist nicht v2');assert(!a1Evidence.includes('s.a1Exam=')&&!a1Evidence.includes('s.a1CanDo.passed='),'A1 Evidence Gate manipuliert Prüfungszustand direkt');
...
// Isolierte Sprech-Inhaltsbewertung: volle einfache Antworten akzeptieren, bloße Keywords nicht.
try{
  const vm=await import('node:vm');const core={normalize:v=>String(v||'').normalize('NFC').toLocaleLowerCase('uk').replace(/[ʼ’‘'`]/g,'’').replace(/[.!?,…:;«»"“”„()]/g,' ').replace(/\s+/g,' ').trim(),recordSession(){}};
  const s={day:3,a1Exam:{start:0,domains:{reading:{passed:true,confirmed:true},listening:{passed:true,confirmed:true},writing:{passed:true,confirmed:true}}},a1HumanSpeaking:{version:1,qualification:null,confirmation:null,best:0,attempts:0,lastAttemptDate:''}};
  const dummy=()=>({className:'',textContent:'',append(){},addEventListener(){},insertAdjacentElement(){}}),document={head:{append(){}},createElement:dummy,getElementById(){return null},querySelectorAll(){return []}};
  const ctx={window:{UKRAINIAN_LEARNING_CORE:core,MediaRecorder:function(){}},navigator:{mediaDevices:{getUserMedia(){}}},MediaRecorder:function(){},s,date:()=> '2026-09-08',save(){},render(){},toast(){},document,Audio:function(){},URL:{revokeObjectURL(){},createObjectURL(){return 'blob:x'}},console};vm.createContext(ctx);vm.runInContext(a1HumanSpeaking,ctx,{filename:'ukrainischkurs-a1-human-speaking-gate.js'});const api=ctx.window.UKRAINIAN_A1_HUMAN_SPEAKING;
  assert(api.evaluate({id:'object'},'Це вода')&&!api.evaluate({id:'object'},'вода'),'Objektantwort verlangt einen einfachen Satz statt Einzelwort');
  assert(api.evaluate({id:'price'},'Це коштує 50 гривень'),'Preisantwort wird nicht erkannt');
  assert(api.evaluate({id:'time'},'Восьма година'),'Zeitantwort wird nicht erkannt');
  assert(api.evaluate({id:'name'},'Мене звати Джоел'),'Namensantwort wird nicht erkannt');
}catch(e){errors.push('A1-Human-Speaking-Runtime: '+(e.stack||e.message))}

for(const [name,source] of [['guided',guided],['writingHardening',writingHardening],['flowHardening',flowHardening],['nativeAudio',nativeAudio],['pronunciation',pronunciation],['mastery',mastery],['humanListening',humanListening],['audioGate',audioGate],['spokenTransfer',spokenTransfer],['a1HumanListening',a1HumanListening],['a1Writing',a1Writing],['a1HumanSpeaking',a1HumanSpeaking],['a1Evidence',a1Evidence]])try{new Function(source)}catch(error){errors.push(`${name} Syntax: ${error.message}`)}
...
try{fs.writeFileSync(swPath,realSw.replace("const VERSION='70'","const VERSION='58'"),'utf8');await import(pathToFileURL(path.join(root,'tests/validate-v58.mjs')).href+'?live='+Date.now())}finally{fs.writeFileSync(swPath,realSw,'utf8')}
console.log(`LIVE-VALIDIERUNG OK: v70 hat ${coverage.toFixed(1)}% Human-Audio im Anfänger-Alphabet, gehärtetes Nachmalen und Tagesnavigation, Human-only A1-Hören, doppelt bestätigtes strukturiertes A1-Schreiben und zusätzlich zweimal 4/4 Human-Audio-Sprechen mit Aufnahme, Rückhören und Verständlichkeitsnachweis. Das zentrale A1-Evidence-Gate verlangt alle vier Evidenzblöcke.`);