/* Ukrainischkurs für Joel · Open Dialogue v2
   Ukrainische Frage verstehen und eine passende ukrainische Antwort selbst erzeugen.
   Keine Antwortbuttons; bekannte Grammatik wird in kurzen Gesprächsfolgen transferiert. */
(()=>{
  const VERSION=2;
  const bridgeStart=Number(s.a1GrammarBridge?.start);
  const timeStart=Number(s.timeBridge?.start);
  ...
  function ensure(){
    if(!s.openDialogue||typeof s.openDialogue!=='object')s.openDialogue={version:VERSION,days:{}};
    if(Number(s.openDialogue.version||0)<VERSION)s.openDialogue={version:VERSION,days:{}};
    s.openDialogue.version=VERSION;s.openDialogue.days=s.openDialogue.days||{};return s.openDialogue
  }
