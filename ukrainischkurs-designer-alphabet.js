...
(()=>{
  ...
  function playLetter(letter,button){stopAudio();const src=window.UKRAINIAN_PRONUNCIATION_AUDIO?.[letter];if(src){const a=new Audio(src);audio=a;button.disabled=true;let settled=false;const done=()=>{if(settled)return;settled=true;clearTimeout(stuck);if(audio===a)audio=null;button.disabled=false};const stuck=setTimeout(()=>{done();toast('Tippe erneut auf Anhören.')},8000);a.onended=done;a.onerror=()=>{if(settled)return;done();toast('Menschliche Buchstaben-Referenz gerade nicht erreichbar.')};a.play().catch(()=>{if(settled)return;done();toast('Tippe erneut auf Anhören.')});return}if(typeof speak==='function')speak(letter,button)}
