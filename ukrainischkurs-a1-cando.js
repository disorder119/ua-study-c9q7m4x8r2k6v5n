/* Ukrainischkurs für Joel · A1 Can-do Check v2
   Abschlussprüfung für konkrete Anfängerhandlungen. Deutlich mehr freie Produktion
   und Hörverstehen; Multiple Choice ist nur noch ein Teil des Nachweises. */
(() => {
  const VERSION=2;
  const TASKS=[
    ...
    {mode:'choice',q:'Du möchtest mit Karte zahlen.',a:'карткою',o:['карткою','дорого','вдома']},
    ...
    {mode:'audio',uk:'Я не можу.',q:'Höre zu. Was bedeutet der Satz?',a:'Ich kann nicht',o:['Ich kann nicht','Ich weiß nicht','Ich will nicht']},
  ...
  function ensure(){
    if(!s.a1CanDo||typeof s.a1CanDo!=='object')s.a1CanDo={version:VERSION,passed:false,best:0,attempts:0,date:'',criticalPassed:false};
    s.a1CanDo.version=VERSION;return s.a1CanDo;
  }
  ...
  function finish(){const st=ensure(),score=Math.round(session.correct/TASKS.length*100),passed=session.correct>=13&&!session.criticalMiss;st.best=Math.max(st.best||0,score);st.attempts=(st.attempts||0)+1;st.date=date();st.criticalPassed=!session.criticalMiss;st.passed=passed;save();session=null;toast(passed?'Can-do-Check bestanden.':'Can-do noch nicht stabil: Ziel 13/16 und alle vier kritischen Selbsthilfe-Aufgaben im ersten Versuch richtig.');render()}
