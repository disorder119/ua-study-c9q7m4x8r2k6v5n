...
(()=>{
  ...
  function play(button){const q=item();if(!q||session.plays>=2)return;const src=source(q);if(!src){toast('Für diese Prüfungsaufgabe fehlt die verifizierte menschliche Quelle. Sie wird nicht als TTS ersetzt.');return}stopAudio();session.heard=false;session.sourceReady=false;button.disabled=true;const a=new Audio(src.url);currentAudio=a;let startSettled=false;const stuck=setTimeout(()=>{if(startSettled)return;startSettled=true;if(currentAudio===a)currentAudio=null;button.disabled=false;session.heard=false;session.sourceReady=false;toast('Menschliche Aufnahme reagiert nicht. Bitte erneut versuchen.');renderGate()},8000);a.onended=()=>{if(currentAudio===a)currentAudio=null;button.disabled=false};a.onerror=()=>{clearTimeout(stuck);startSettled=true;if(currentAudio===a)currentAudio=null;button.disabled=false;session.heard=false;session.sourceReady=false;toast('Menschliche Aufnahme konnte nicht geladen werden. Kein TTS-Ersatz in der Prüfung.');renderGate()};const p=a.play();const ok=()=>{clearTimeout(stuck);startSettled=true;session.plays++;session.heard=true;session.sourceReady=true;button.disabled=false;renderGate()};if(p&&typeof p.then==='function')p.then(ok).catch(()=>{clearTimeout(stuck);startSettled=true;button.disabled=false;session.heard=false;session.sourceReady=false;toast('Menschliche Aufnahme konnte nicht gestartet werden. Kein TTS-Ersatz in der Prüfung.');renderGate()});else ok()}
