/* Ukrainischkurs für Joel · Free Reading Transfer v1
   Kurze unbekannte Kombinationen aus bekanntem Stoff. Text wird nach dem Lesen
   ausgeblendet; Verständnis muss anschließend frei auf Ukrainisch formuliert werden. */
(()=>{
  const VERSION=1;
  ...
  function answer(value){const q=session.item.qs[session.idx],good=q.a.map(norm).includes(norm(value));if(good)session.correct++;toast(good?'Verstanden.':'Eine passende Antwort ist: '+q.a[0]);session.idx++;if(session.idx>=session.item.qs.length){const st=ensure(),perfect=session.correct===session.item.qs.length,score=Math.round(session.correct/session.item.qs.length*100);st.best=Math.max(st.best||0,score);st.attempts++;st.date=date();st.passed=perfect;save();session=null;toast(perfect?'Freier Lese-Transfer bestanden.':'Noch nicht stabil: neuer Text, neuer frischer Versuch.');render();return}renderBox()}
