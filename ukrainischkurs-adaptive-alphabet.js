...
(() => {
  ...
  function playAudio(letter,button){const src=window.UKRAINIAN_PRONUNCIATION_AUDIO?.[letter];if(src){const a=new Audio(src);button.disabled=true;let settled=false;const release=()=>{if(settled)return;settled=true;clearTimeout(stuck);button.disabled=false};const stuck=setTimeout(()=>{release();toast('Tippe erneut auf Anhören.')},8000);a.onended=release;a.onerror=()=>{if(settled)return;release();toast('Menschliche Referenz gerade nicht erreichbar.')};a.play().catch(()=>{if(settled)return;release();toast('Tippe erneut auf Anhören.')});return}if(typeof speak==='function')speak(letter,button)}
