/* Ukrainischkurs für Joel · Conversation Chain v4
   Mehrteilige Gespräche mit sichtbarem Verlauf. Alle Ketten hängen an konkreten
   eingeführten Lernobjekten; Bewertung läuft ausschließlich über den Lernkern. */
(()=>{
  const VERSION=4,core=window.UKRAINIAN_LEARNING_CORE;if(!core)return;
  const CHAINS={};
  const attach=(requirements,items)=>{const day=core.anchorDay(requirements);if(Number.isFinite(day)&&day>=0)CHAINS[day]=items};
  attach(['Він з Києва'],[
  ...
  attach(['п’ять квитків','Я буду працювати'],[
    {q:'Звідки ти?',a:['Я з Німеччини'],hint:'Ich komme aus Deutschland.'},
    {q:'Де ти зараз?',a:['Я в Києві','Я у Києві'],hint:'Ich bin jetzt in Kyjiw.'},
    {q:'Що ти хочеш?',a:['Я хочу каву'],hint:'Ich möchte Kaffee.'},
    {q:'Скільки це коштує?',a:['Це десять гривень','Десять гривень'],hint:'Das kostet zehn Hrywnja.'},
    {q:'У тебе є квиток?',a:['У мене немає квитка','Ні, у мене немає квитка'],hint:'Ich habe kein Ticket.'},
  ...
  function answer(value){const item=session.items[session.idx],good=core.accepts(value,item.a);session.history.push({q:item.q,a:value||'—',good});if(good)session.correct++;toast(good?'Passt.':'Eine passende Antwort ist: '+item.a[0]);session.idx++;if(session.idx>=session.items.length){const st=state(),perfect=session.correct===session.items.length,score=Math.round(session.correct/session.items.length*100);st.best=Math.max(st.best||0,score);st.attempts++;st.date=date();st.passed=perfect;core.recordSession({skills:['reading','writing','grammar'],correct:session.correct,total:session.items.length,passed:perfect,module:'conversation-chain',day:s.day,assisted:session.listened>0});session=null;toast(perfect?'Gesprächskette bestanden.':'Noch nicht stabil: alle Antworten müssen in einem frischen Gespräch passen.');render();return}renderBox()}
  ...
  window.UKRAINIAN_CONVERSATION_CHAIN={version:VERSION,maxTurns:Math.max(...Object.values(CHAINS).map(x=>x.length)),dynamicAnchors:true,centralScoring:true};const previousRender=render;render=function(){previousRender();renderBox()};ensure();renderBox();
})();