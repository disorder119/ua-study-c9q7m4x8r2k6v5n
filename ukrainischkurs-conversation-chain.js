/* Ukrainischkurs für Joel · Conversation Chain v1
...
(()=>{
  ...
  function renderBox(){let box=document.getElementById('conversationChainBox');if(!required()){if(box)box.hidden=true;return}const cards=document.getElementById('cards');if(!cards)return;if(!box){box=document.createElement('section');box.id='conversationChainBox';box.className='card';cards.insertAdjacentElement('afterend',box)}box.hidden=false;const st=state();if(session){const item=session.items[session.idx];box.innerHTML='<div class="label">Gesprächskette · '+(session.idx+1)+' / '+session.items.length+'</div><h2>Ein Gespräch zusammenhalten</h2>'+historyHtml()+'<div class="cc-current"><button class="secondary" id="ccListen">🔊</button><span lang="uk">'+item.q+'</span></div><p class="small">'+item.hint+'</p><input id="ccInput" class="typing-input" lang="uk" autocapitalize="off" autocorrect="off" autocomplete="off" spellcheck="false" placeholder="Auf Ukrainisch antworten …"><div class="actions"><button class="primary" id="ccCheck">Antwort senden</button></div>';const inp=document.getElementById('ccInput');document.getElementById('ccCheck').onclick=()=>answer(inp.value);document.getElementById('ccListen').onclick=e=>speak(item.q,e.currentTarget);inp.onkeydown=e=>{if(e.key==='Enter'){e.preventDefault();answer(inp.value)}};setTimeout(()=>inp.focus(),0)}else box.innerHTML='<div class="label">Gespräch · 4 zusammenhängende Züge</div><h2>Nicht vier Einzelkarten, sondern ein Gespräch</h2><p class="small">Die bisherigen Antworten bleiben sichtbar. So musst du im selben Szenario Ort, Richtung, Genitiv und Zeitformen zusammen benutzen.</p><div class="tip">'+(st.passed?'✓ Vier passende Antworten in einem Gespräch.':'Bestehen: 4/4 in einem frischen Durchgang.')+'</div><div class="actions"><button class="'+(st.passed?'secondary':'primary')+'" id="ccStart">'+(st.passed?'noch einmal':'Gespräch starten')+'</button></div>';const btn=document.getElementById('ccStart');if(btn)btn.onclick=startChain}
