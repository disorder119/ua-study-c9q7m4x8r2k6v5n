/* Ukrainischkurs für Joel · Exam Dashboard v1
...
(()=>{
  ...
  const clamp=(n,a,b)=>Math.max(a,Math.min(b,n)),shuffle=a=>[...a].sort(()=>Math.random()-.5),pick=a=>a[Math.floor(Math.random()*a.length)];
  ...
  function introducedCards(){
    const upto=clamp(Number(s.day)||0,0,D.length-1),out=[];
  ...
  function mockDomainAverage(skill,n=5){const vals=recentHistory(n).map(x=>Number(x.domains?.[skill]?.score)).filter(Number.isFinite);return vals.length?Math.round(vals.reduce((a,b)=>a+b,0)/vals.length):null}
  function skillSignal(skill){
    ...
    const status=evidence<2?'grey':score>=80?'green':score>=60?'yellow':'red';
    return {skill,label:LABELS[skill],score,status,evidence};
  }
  function signals(){return SKILLS.map(skillSignal)}
  function coursePercent(){return Math.round(clamp(((Number(s.day)||0)+1)/Math.max(1,D.length)*100,0,100))}
  ...
  function learningPercent(){
    const vals=signals().map(x=>x.score).filter(Number.isFinite);const skills=vals.length?vals.reduce((a,b)=>a+b,0)/vals.length:0,course=coursePercent(),mock=simulationAverage();
  ...
  function recommendedMode(){const sig=signals();if(sig.some(x=>x.status==='red'))return'weak';const h=history(),lastFull=[...h].reverse().find(x=>x.mode==='full');if((Number(s.day)||0)>=24&&(!lastFull||(Number(s.day)||0)-Number(lastFull.day||0)>=12))return'full';return'standard'}
  function simulationDue(){const h=history(),last=h[h.length-1];if((Number(s.day)||0)<8)return false;if(WEEKLY_REVIEW_DAYS.includes(Number(s.day)))return true;return !last||(Number(s.day)||0)-Number(last.day||0)>=4}
  ...
  function distractors(card,pool){const vals=shuffle(pool.filter(x=>x.de!==card.de).map(x=>x.de));return [...new Set(vals)].slice(0,3)}
  ...
  function makeQuestion(skill,pool,used){
    const c=cardForSkill(skill,pool,used);if(!c)return null;
    const base={skill,uk:c.uk,de:c.de,answer:c.uk,plays:0,assisted:false,spoken:false,user:'',correct:false};
  ...
  function createSession(mode){
    const cfg=MODES[mode]||MODES.standard,pool=introducedCards();if(pool.length<8){toast('Für Prüfungssimulationen brauchst du erst etwas mehr eingeführten Lernstoff.');return null}
    const used=new Set(),questions=domainPlan(cfg.id).map(k=>makeQuestion(k,pool,used)).filter(Boolean);return {mode:cfg.id,questions,idx:0,startedAt:Date.now(),answers:[],finished:false}
  }
  ...
  function renderDashboard(){
    ...
    const hist=recentHistory(5).reverse();const histHtml=hist.length?hist.map((x,i)=>'<div class="ed-history"><span>'+esc(MODES[x.mode]?.title||'Simulation')+'</span><strong>'+x.score+'%</strong><small>'+esc(x.date||'')+(i===0?' · zuletzt':'')+'</small></div>').join(''):'<p class="small">Noch keine Prüfungssimulation abgeschlossen.</p>';
  ...
  function renderDueCard(){
    const learn=document.getElementById('learn');if(!learn)return;let box=document.getElementById('examDueCard');if(!simulationDue()){if(box)box.remove();return}if(!box){box=document.createElement('article');box.id='examDueCard';box.className='card ed-due-card';const first=learn.querySelector('.card');first?.insertAdjacentElement('afterend',box)}
    const mode=recommendedMode();box.innerHTML='<div class="label">Prüfungstraining fällig</div><h2>Kurz testen, was wirklich sitzt</h2><p class="small">Wie bei einer Führerschein-Lernapp kommt regelmäßig eine Simulation. Sie ist Übung und verändert deine echte A1-Prüfung nicht.</p><div class="actions"><button class="primary" id="edDueStart">'+esc(MODES[mode].title)+' starten</button><button class="secondary" id="edDueProgress">Lernampel ansehen</button></div>';
    box.querySelector('#edDueStart').onclick=()=>startSimulation(mode);box.querySelector('#edDueProgress').onclick=()=>showProgress();
  }
  ...
  function playListening(q,button){if(q.plays>=2){toast('Maximal zwei Wiedergaben in dieser Simulation.');return}q.plays++;if(typeof speak==='function'){try{speak(q.uk,button);renderQuestion();return}catch{}}q.assisted=true;toast('Keine Sprachausgabe verfügbar. Der Text wird als unterstützter Fallback gezeigt.');renderQuestion()}
  function submitChoice(value){const q=session?.questions?.[session.idx];if(!q)return;q.user=value;q.correct=value===q.correctValue;session.answers.push({...q});session.idx++;session.idx>=session.questions.length?finishSimulation():renderQuestion()}
  ...
  function renderQuestion(){
    ...
    else if(q.type==='listening')body='<div class="ed-listen"><button class="primary" id="edPlay">🔊 anhören ('+q.plays+'/2)</button>'+(q.assisted?'<div class="tip"><strong>Unterstützter Fallback:</strong> <span lang="uk">'+esc(q.uk)+'</span></div>':'')+'</div><p class="q">'+esc(q.prompt)+'</p><div class="answers">'+q.options.map(x=>'<button class="answer" data-choice="'+esc(x)+'">'+esc(x)+'</button>').join('')+'</div>';
  ...
  function finishSimulation(){
    const cfg=MODES[session.mode],answers=session.answers,total=answers.length,correct=answers.filter(x=>x.correct).length,score=Math.round(correct/Math.max(1,total)*100),domains={};
    SKILLS.forEach(k=>{const a=answers.filter(x=>x.skill===k),c=a.filter(x=>x.correct).length;if(a.length)domains[k]={correct:c,total:a.length,score:Math.round(c/a.length*100),assisted:a.some(x=>x.assisted)} });
    const minDomain=Math.min(...Object.values(domains).map(x=>x.score)),passed=score>=cfg.threshold&&(session.mode!=='full'||minDomain>=60),entry={id:Date.now(),date:typeof date==='function'?date():new Date().toISOString().slice(0,10),day:Number(s.day)||0,mode:session.mode,score,correct,total,passed,domains,durationSec:Math.round((Date.now()-session.startedAt)/1000)};
    ...
    answers.filter(x=>!x.correct&&['writing','grammar','speaking'].includes(x.skill)).forEach(x=>window.UKRAINIAN_ERROR_MEMORY?.record?.({input:x.user,answers:[x.answer],prompt:x.de,correct:false,module:'practice-exam-simulation',day:Number(s.day),weight:.35}));
    save();session.finished=true;renderResult(entry,answers);session=null;renderDashboard();renderDueCard();
  }
  function renderResult(entry,answers){
    const box=ensureExamBox(),cfg=MODES[entry.mode],domains=SKILLS.map(k=>{const d=entry.domains[k];if(!d)return'';const light=lightForScore(d.score,2);return '<div class="ed-result-domain '+light+'"><span class="ed-dot"></span><strong>'+LABELS[k]+'</strong><b>'+d.score+'%</b></div>'}).join('');
    const mistakes=answers.filter(x=>!x.correct);const mistakeHtml=mistakes.length?mistakes.slice(0,12).map(x=>'<div class="ed-mistake"><small>'+esc(LABELS[x.skill])+'</small><strong>'+esc(x.de)+'</strong><span lang="uk">'+esc(x.answer)+'</span>'+(x.user?'<em>Deine Antwort: '+esc(x.user)+'</em>':'')+'</div>').join(''):'<div class="tip">Keine Fehler in dieser Simulation.</div>';
    box.hidden=false;box.innerHTML='<div class="label">Auswertung · '+esc(cfg.title)+'</div><div class="ed-result-head"><div class="ed-score '+(entry.passed?'green':'yellow')+'"><strong>'+entry.score+'%</strong><span>Ergebnis</span></div><div><h2>'+(entry.passed?'Übung bestanden':'Noch nicht stabil genug')+'</h2><p class="small">'+entry.correct+' von '+entry.total+' richtig. Bestehensziel für diese Übung: '+cfg.threshold+'%'+(entry.mode==='full'?' und kein Bereich unter 60%':'')+'.</p></div></div><div class="ed-result-domains">'+domains+'</div><h3>Fehler ansehen</h3>'+mistakeHtml+'<div class="actions"><button class="primary" id="edAgain">Neue '+esc(cfg.title)+'</button><button class="secondary" id="edWeak">Schwächenprüfung</button></div><p class="small">Diese Auswertung ist ein Lernindikator. Sie ersetzt weder die spätere A1-Abschlussprüfung noch eine offizielle Sprachprüfung.</p>';
    box.querySelector('#edAgain').onclick=()=>startSimulation(entry.mode);box.querySelector('#edWeak').onclick=()=>startSimulation('weak');box.scrollIntoView({behavior:'smooth',block:'start'});
  }
